    ...
    while (SDL_PollEvent(&e)) {
      ...
      case SDL_MOUSEBUTTONDOWN:
        ...
        if (cursorInGrid && componentCount <= 255 && !simulating) {
          ...
          int w, h;
          GetWidthHeight(&w, &h, selectedComponent.type,
                         selectedComponent.size);
          if (!WireIsValid(grid, gridPos, x, y, pad_x, pad_y) &&
              cell(gridPos.y, gridPos.x) >= 0) {
            if (ComponentList[cell(gridPos.y, gridPos.x)].type == state)
              ComponentList[cell(gridPos.y, gridPos.x)].operate(
                  &ComponentList[cell(gridPos.y, gridPos.x)]);
          }
          if (!drawingWire &&
              PositionIsValid(grid, w, h, selectedComponent.pos)) {
